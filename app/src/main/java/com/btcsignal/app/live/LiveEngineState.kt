...
object LiveEngineState {
     ...
     *  of which screen is on-screen -- unlike [priceMovePoints] above, which only walks
     *  while PriceMoveChart is actually composed. Read once at candle close to snapshot
     *  the signal's chart for History (see PricePathSnapshot.kt); always access under
     *  `synchronized(candlePriceLog)` since writes can come from the WebSocket callback
     *  thread while a read is building the snapshot JSON. */
