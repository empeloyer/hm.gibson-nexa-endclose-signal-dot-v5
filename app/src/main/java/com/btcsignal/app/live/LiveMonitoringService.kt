...
class LiveMonitoringService : Service(), BinanceStreamListener {
    ...
    override fun onKlineUpdate(candle: Candle) {
        ...
        if (!candle.isClosed && aggregator.isNewBucket(candle)) {
            ...
            // Fresh chart-snapshot log for the new candle (see candlePriceLog KDoc).
        ...
        if (openTime > 0) {
            ...
            synchronized(LiveEngineState.candlePriceLog) {
                ...
                if (LiveEngineState.candlePriceLog.size > 600) LiveEngineState.candlePriceLog.removeAt(0)
            }
        }
    ...
    private suspend fun handleCandleClosed(candle: Candle) {
        ...
        if (existing != null && existing.candleId == candleId) {
            ...
            // Freeze this candle's chart-snapshot path onto the signal row (see
            // candlePriceLog KDoc / PricePathSnapshot.kt) so History can redraw it later.
